...
// 出口代理适配：若环境配置了 HTTP(S)_PROXY（部分部署/沙箱），则抓取走代理
...
app.get('/api/weather', async (req, res) => {
  try {
    ...
    res.json({ ok: true, location: WEATHER.name, data });
  } catch (e) {
    res.status(502).json({ ok: false, error: '天气服务暂时不可用: ' + e.message });
  }
});
...
// 根据主体/对象/事件，生成贴合本条新闻的影响（对工作 + 对生活 + 行动）
const OBJ_IMPACT = {
...
function contentImpact(title, cat){
  ...
  // 无细分对象时，仍引用本条标题内容，避免用"综合类空话"敷衍
  const c = classify(title);
  return '本条关于「' + title.slice(0, 20) + (title.length>20?'…':'') + '」的新闻，对工作：' + (c.work || '这类消息反映行业或宏观的新动向，相关从业者可据此评估本岗位是否受影响，并留意后续正式细则与官方解读。') + '；对生活：' + (c.life || '建议结合自身处境，重点吸收与个人利益相关的部分，理性看待，不必过度解读。') + '；' + (c.act || '行动建议：关注与其直接相关的本地落地方案。');
}
// 内容化"要点"，保证不是标题+标点+套话，而是有信息量的条目
...
function makeItem(title, link, source, seen, section) {
  ...
  // 递归实体解码并清除注释/模板标记（&amp; &#x5e74; [!--...--] 等 → 正常汉字），防止显示"代码"
  ...
  // 影响栏改为展示"新闻原文"：先以智能生成的原文兜底，后台抓到详情正文后替换
  const fallbackImpact = genFallbackImpact(tt, sec);
...
// 为新闻条目生成基于标题的兜底原文（不再只写标题，至少给一段有信息量的描述）
...
function catLife(section, t) {
  const c = classify(t);
  if (section === '青岛') return '青岛本地新动态，与日常出行、通勤、消费购物和工作安排都可能直接相关，建议结合自身情况及时跟进，留意它对生活与工作节奏带来的实际影响。';
  if (section === '文娱八卦') return '文娱圈的近期话题，反映大众关注焦点，工作之余可作谈资，也能帮助把握当下的内容与消费风向；但娱乐消费请量力而行、认准官方渠道。';
  if (section === '热搜榜') return '全网热搜热点，体现用户关注趋势与情绪走向，可为内容与营销选题提供参考；关注热点时也请留意其真实性。';
  if (section === '国际') return '国际形势的动态变化，多通过贸易、汇率、出行与产业链影响国内工作和生活，涉外、外贸与跨境从业者需重点关注，普通人应及时了解以避免误判。';
  if (c.cat === '经济产业') return '反映经济与市场的最新走向，直接影响你的收入、就业选择与消费决策，值得针对与自身相关的一两部分重点跟进。';
  if (c.cat === '政策法规') return '政策与法规新动态，往往带有强制性和时序性，与日常办事、合规要求和权益维护相关，建议关注官方原文与地方细则的落地执行。';
  if (c.cat === '民生健康') return '切实关系到就医、养老、住房、生育等切身利益的民生内容，建议及时了解本地实施细则，把相关权益和申报窗口用足用好。';
  if (c.cat === '科技航天') return '科技进步带来新的发展机遇与生活便利，相关行业技能需求进一步上升，关注其进展有助于把握新趋势、做好学习与职业规划。';
  if (c.cat === '安全法治') return '关乎公共秩序与信息财产安全，提醒增强防范意识、保护个人隐私与账号，遇到可疑情况依法处置、及时报警。';
  return '这类综合信息与工作和生活都可能相关，既反映大环境也关联个人处境，建议理性看待、结合自身实际去粗取精即可。';
}
function todayStr() {
  const d = new Date();
...
// ============ 央视早/午/晚间新闻联播（24小时内）—— 国内板块专用 ============
// 来源：
//   早间《朝闻天下》06:00  JSON: TOPC1451558496100826
//   午间《新闻30分》12:00  JSON: TOPC1451559097947700
//   晚间《新闻联播》19:00  HTML: tv.cctv.com/lm/xwlb/day/YYYYMMDD.shtml
const XWLB_COLUMNS = {
...
async function crawlXinwenLianbo(out, seen) {
  ...
  // 1) 晚间《新闻联播》day 页面（当天 + 前一天，覆盖 24 小时）
  ...
  // 2) 早间《朝闻天下》 + 午间《新闻30分》JSON 接口（brief 按分号拆成多条）
...
// 新闻联播条目专用创建（标题可较长，不走 makeItem 的 38 字限制）
...
// B站条目专用构造：标题可长、可含逗号/句号（区别于 makeItem 的 38 字与标点过滤）
// 从 API 提取视频描述(description)作为新闻原文，解决"B站只有标题没有内容"的问题
function makeBiliItem(v, seen) {
  ...
  // 描述足够且有信息量时用之；否则用基于标题分类生成的有信息量原文，保证每条都不只是标题
  const impact = (desc && desc.length >= 30) ? desc : genBiliLead(tt);
  // 要点=正文：用真实描述替代模板生成的要点（至少200字）
...
// 抓取 B站热门视频榜（大家关注的科技/生活/知识话题）
// 修复：ranking/v2 接口易返 -352 风控，改用 popular/ranking 接口并带 Referer 绕过
...
// 抓取微博热搜榜（官方 hot_band 接口，带 Referer 绕过风控）
...
// 抓取小红书热搜榜（46.la 榜单页，含排名与热度）
async function crawlXiaohongshuHot(out, seen) {
  try {
    // 用浏览器 UA + Referer 提高命中率
    const controller = new AbortController();
...
function ensurePerSection(live, date) {
  ...
  // 每个板块至少 MIN 条：优先再来一轮实时中同板块尚未取到的（已全量加入），再用内置补齐
...
// ============ 新闻缓存 + 定时刷新（每天 08:00 / 20:00 各更新一遍） ============
let newsCache = null; // { items, crawled, updatedAt, builtDate }
let refreshing = Promise.resolve();
// 24 小时新闻池：把历次刷新抓到的条目累积、去重、按时间倒序，让"24 小时内"都有内容可看（不只实时那一批）
let newsPool = []; // { item, ts } 按 ts 倒序
const NEWS_24H_MS = 24 * 60 * 60 * 1000 + 10 * 60 * 1000; // 24h + 10min 缓冲
function mergeIntoPool(items, date) {
  const now = Date.now();
  const map = new Map(); // title -> { item, ts }
  // 新抓到的条目优先并入（时间取当前，保证是最新），让它们排在最前不会被截掉
  for (const it of items) {
    if (!map.has(it.title)) map.set(it.title, { item: it, ts: now });
  }
  // 再保留池中仍在 24h 内的旧条目（仅补全未覆盖的标题）
  for (const e of newsPool) {
    if (now - e.ts <= NEWS_24H_MS) { if (!map.has(e.item.title)) map.set(e.item.title, e); }
  }
  // 重排序：ts 越大越靠前；同样时间戳的新条目保持插入顺序（新在前）
  newsPool = Array.from(map.values()).sort((a, b) => b.ts - a.ts).slice(0, 100);
}

// 抓取新闻详情原文：多策略提取正文作为"新闻原文"填充 impact，失败则保留已有内容兜底
async function enrichNewsDetail(it) {
  if (!it || !it.link || !/^https?:\/\//.test(it.link)) return;
  // B站：如果已有 description（_hasDesc），不覆盖；否则走兜底生成
  if (/bilibili/.test(it.link)) {
    if (!it._hasDesc && it.title) it.impact = genBiliLead(it.title);
    return;
  }
  // 如果已有从 API 带过来的原文（非标题），且长度足够，跳过
  ...
  try {
    ...
    // 策略3：div class 含 content/article/text/body
    if (leads.length < 2) {
...
async function doRefreshNews() {
  const date = todayStr();
  // 注意：seen 只用于"实时抓取内部去重"，不要预置内置标题，
  // 否则当内置热搜标题与实时榜单标题相同时，实时条目会被误判为重复而丢弃。
  const seen = new Set();
  const live = [];
  ...
  // 后台为每条新闻补抓原文正文：按板块均衡分配，确保各板块都有原文（失败不影响展示）
...
function scheduleNextNewsRefresh() {
  ...
  d.setHours(8, 0, 2, 0); if (d > now) targets.push(d);
  ...
  netClear = setTimeout(() => {
    doRefreshNews().then(r => { newsCache = r; if (process.env.DEBUG_NEWS) console.log('[news] 定时刷新完成', r.crawled); })
      .catch(() => { /* 保底：保留旧缓存 */ })
...
// 启动即抓取一次；随后按早/晚时间点自动刷新
doRefreshNews().then(r => { newsCache = r; if (process.env.DEBUG_NEWS) console.log('[news] 启动刷新完成', r.crawled); })
  .catch(() => { /* 暂时无缓存，请求时兜底 */ })
  .finally(scheduleNextNewsRefresh);

app.get('/api/news', async (req, res) => {
  const date = todayStr();
  // 若缓存有效且是今天构建的，直接返回（快、稳定）；否则走一次兜底
  if (newsCache && newsCache.builtDate === date) {
    return res.json({
      ok: true, date, cached: true, updatedAt: newsCache.updatedAt,
      channels: ['央视新闻', '新华网', '澎湃新闻', '网易娱乐', 'B站热门', '微博热搜', '小红书热搜', '青岛新闻网', '权威媒体'],
      sections: NEWS_SECTIONS,
      crawled: newsCache.crawled,
      items: newsCache.items
    });
  }
  try {
    const r = await doRefreshNews(); newsCache = r; scheduleNextNewsRefresh();
    res.json({
      ok: true, date, cached: false, updatedAt: r.updatedAt,
      channels: ['央视新闻', '新华网', '澎湃新闻', '网易娱乐', 'B站热门', '微博热搜', '小红书热搜', '青岛新闻网', '权威媒体'], sections: NEWS_SECTIONS, crawled: r.crawled, items: r.items
    });
  } catch (e) {
    if (process.env.DEBUG_NEWS) console.error('[news] refresh fail:', e && e.message);
    res.json({
      ok: true, date, cached: false, sections: NEWS_SECTIONS,
      channels: ['权威媒体'],
...
// ============ 青岛周边游实时攻略（多源尽力抓取 + 后台定时刷新缓存） ============
// 说明：以"青岛及周边短途游"为主题，主实时源用搜索引擎实时结果（Bing 对机器人较宽松），
//       按青岛旅游相关关键词检索必去景点/行程/攻略并附链接；内置精选始终兜底，保证任何情况都有内容。
...
const DEAL_KEYS_ON = ['青岛','崂山','北九水','避暑','玩水','海边','泰山','长白山','草原','坝上','承德','呼伦贝尔','青海','贵州','烟台','威海','日照','夏日','盛夏','8月','暑假','夏季','亲子','推荐','攻略','景点','好玩','首发','路线'];
const BRAND_KEYS_ON = ['山姆','盒马','京东','淘宝','天猫','拼多多','云闪付','银行','利群','家家悦','美团','支付宝','会员','返利','券','特价','折扣'];
...
// 从 Bing 结果页解析 {标题, 链接, 摘要}，把"摘要"作为真实省钱技巧正文，而非面上套话
function parseBingResults(html){
  ...
  while ((li = liRe.exec(body)) && out.length < 14) {
    ...
    // 负向：剔除百科/政务/企业名录/官网首页/游戏/导航/非旅游等无关项
    ...
    // 正向：必须与"避暑/游玩/景点/行程/目的地"相关，否则丢弃
    ...
    // 摘要即真实内容；不要面上的套话
    const body = (snippet && snippet.length >= 12) ? '摘要：' + snippet : '网络实时搜到的当季暑期出行信息，点开链接看详情。';
    out.push({ t: title, link, source: '实时攻略', snippet, live: true, d: body, how: '点开链接看完整玩法与交通住宿建议；无账号也能先看摘要，无需强制登录' });
  }
  return out;
}
async function crawlBingCoupons(out){
  ...
  const job = async (url) => {
    ...
    try {
      const resp = await fetch(url, { signal: controller.signal, headers: { 'User-Agent': UA_DESKTOP }, ...(hasProxy() ? { dispatcher: new ProxyAgent(PROXY) } : {}) });
      if (!resp.ok) return [];
...
// 当季内置精选（服务端兜底 + 保证实时噪声不淹没优质当季内容）：按季节人工整理
...
async function refreshCoupons(){
  ...
  couponCache = { items: items.slice(0, 24), updatedAt: Date.now(), crawled: items.length };
  console.log('[coupons] refreshed', items.length, 'items');
}
